        ...
        if (mMediaPlayer == null) {
            switch (mPlayerType) {
                ...
                default:
                    ...
                    //设置最大缓存
                    ((IjkMediaPlayer) mMediaPlayer).setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "packet-buffering", 1L);
                    ...
                    ((IjkMediaPlayer) mMediaPlayer).setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "min-frames", 100);
//                    ((IjkMediaPlayer) mMediaPlayer).setOption(IjkMediaPlayer.OPT_CATEGORY_FORMAT,"headers", "User-Agent:Mozilla/5.0 (iPhone; CPU iPhone OS 9_1 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Version/9.0 Mobile/13B143 Safari/601.1");
                    break;
            }
...
//        NiceUtil.hideActionBar(mContext);
        ...
        LayoutParams params = new LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT);
        int i = DensityUtil.dip2px(mContext, 20);
        params.bottomMargin = -i;
        LinearLayout bottom = ((VideoPlayerController) mController).getBottom_();
        bottom.setPadding(0, 0, 0, i);
